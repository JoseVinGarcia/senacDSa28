...
try:
    ...
    #filtra os dados no segundo turno
    # filtra os dados que estao entre os candidatos 13 e 22
    ...
    with pl.StringCache():
        ...
        '''VOTAÇÃO'''
        # lazy com delimitação das colunas. Pega só o schema dos dados
    ...
    pl.Config.set_float_precision(2) #duas casas decimais
    pl.Config.set_decimal_separator(',') # separador decimal
    pl.Config.set_thousands_separator('.') #Separador de milhar
...
try:
    ...
    # diconário candidato:correlacao(para o nome dos presidentes)
    dict_correlacoes = {}

    for candidato in df_votos_bolsa_familia['NM_VOTAVEL'].unique(): # para cada candidato na minha lista de candidato
        #filtrar dados pelo candidato
        df_candidato = df_votos_bolsa_familia.filter(pl.col('NM_VOTAVEL') == candidato) # pega o nome se for igual ao que esta no for

        # Arrays com as variáveis quantitativas do candidato filtrado (pega todas as quantidades de votos e as parcelas)
